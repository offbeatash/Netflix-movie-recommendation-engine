...
def create_splits():
    ...
    print("Calculating time-based quantiles for splitting FIRST to prevent temporal leakage...")
    q80 = df["Date"].quantile(TRAIN_SPLIT_QUANTILE)
    q90 = df["Date"].quantile(VAL_SPLIT_QUANTILE)

    print("Filtering inactive users and movies strictly based on the training period...")
    min_user_rating = 10
    min_movie_rating = 50
    ...
    print(f"Physically splitting data ({TRAIN_SPLIT_QUANTILE*100}% Train | {(VAL_SPLIT_QUANTILE-TRAIN_SPLIT_QUANTILE)*100}% Val | {(1-VAL_SPLIT_QUANTILE)*100}% Test)...")
