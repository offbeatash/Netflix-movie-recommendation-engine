...
def process_enrichment():
    ...
    if ENRICHED_MOVIES_PATH.exists():
        print(f"Enriched movies already exist at {ENRICHED_MOVIES_PATH}. Skipping API calls.")
    ...
    for start in range(0, len(missing_idx), batch_size):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            results = list(tqdm(executor.map(fetch_worker, batch_indices), total=len(batch_indices)))
