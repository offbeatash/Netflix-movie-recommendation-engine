...
def evaluate_models():
    ...
    svd_alpha = ensemble_weights.get(
        "svd_alpha", ensemble_weights.get("best_alpha", 0.0)
    )
    ...
    valid_users = (test_df["user_idx"].values >= 0) & (
        test_df["user_idx"].values < n_users_als
    )
    valid_movies = (test_df["movie_idx"].values >= 0) & (
        test_df["movie_idx"].values < n_movies_als
    )
    ...
    u_factors[valid_mask] = als_model.user_factors[
        test_df["user_idx"].values[valid_mask]
    ]
    m_factors[valid_mask] = als_model.item_factors[
        test_df["movie_idx"].values[valid_mask]
    ]
    ...
    pred_ensemble = np.clip(
        svd_alpha * pred_svd + (1.0 - svd_alpha) * pred_pop, 1.0, 5.0
    )
        ...
        mlflow.log_metrics(
            {
                ...
                "ensemble_test_rmse": float(
                    metrics_by_model.loc["Model D (Ensemble)", "RMSE"]
                ),
                "ensemble_test_mae": float(
                    metrics_by_model.loc["Model D (Ensemble)", "MAE"]
                ),
            }
        )
    ...
    print(
        (
            "Note: The extreme error in the ALS model demonstrates the "
            "Implicit vs. Explicit Feedback Trap."
        )
    )
