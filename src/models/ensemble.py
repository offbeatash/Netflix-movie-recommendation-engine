...
def get_or_train_ensemble(force_retrain=False):
    """Calculate the optimal ALS/SVD blend and return its alpha weight."""
    ...
    pred_pop = (
        val_df["Movie_ID"]
        .map(popularity_artifact["movie_avgs"])
    ...
    print(
        f"Optimization complete! Optimal Blend: {svd_weight}% SVD + "
        f"{100 - svd_weight}% popularity"
    )
